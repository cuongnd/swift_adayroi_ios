...
class Cart: Model {
    ...
    func toString(cart:Row) {
        print("Cart detail id=\(cart[self.id]), _id=\(cart[self._id]), cat_id=\(cart[self.cat_id]), sub_cat_id=\(cart[self.sub_cat_id]), original_price=\(cart[self.original_price]), unit_price=\(cart[self.unit_price]), discount_amount=\(cart[self.discount_amount]), currency_symbol=\(cart[self.currency_symbol]), discount_percent=\(cart[self.discount_percent]), color_id=\(cart[self.color_id]), color_name=\(cart[self.color_name]), quality=\(cart[self.quality])")
    }
    func insert(_id:String,cat_id:String,sub_cat_id:String,original_price:Int64,unit_price:Int64,name:String,discount_amount:Int64,currency_symbol:String,discount_percent:Int64,color_id:String,color_name:String,quality:Int64 ) -> Int64? {
        do{
